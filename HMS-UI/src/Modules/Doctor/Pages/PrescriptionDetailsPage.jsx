...
import PrescriptionDetailsApi from "../services/PrescriptionDetailsApi";
...
const PrescriptionDetailsPage = () => {
  const { data, loading, error, createItem, updateItem, deleteItem } =
    useCrud(PrescriptionDetailsApi);
  ...
  // From Fields
  const PrescriptionDetailsFields = [
    {
      name: "visitDate",
      label: "Visit Date",
      type: "date",
  ...
  // Table Columns
  const PrescriptionDetailsColumns = [
    { key: "id", label: "ID" },
  ...
  return (
    <Layout>
      ...
      <ReusableForm
        ...
        submitText={editing ? "Update PrescriptionDetails" : "Register PrescriptionDetails"}
      />
      ...
      {loading ? (
        ...
        <DataTable
          columns={PrescriptionDetailsColumns}
          data={data}
          onEdit={setEditing}
          onDelete={deleteItem}
        />
