...
public class PharmacySaleDetailsSpecification : BaseSpecification<PharmacySalesdetails>
{
    public PharmacySaleDetailsSpecification(QueryParams query)
    {
        /* ---------- include Medicine and Prasmacy sale ---------- */
        AddInclude(m => m.Medicine);
        AddInclude(p => p.PharmacySale);
            ...
            switch (query.Sorting.SortBy.ToLower())
            {
                case "name":
                    if (query.Sorting.IsDescending)
                        ApplyOrderByDescending(d => d.Medicine);
                    else
                        ApplyOrderBy(d => d.Medicine);
                    break;
